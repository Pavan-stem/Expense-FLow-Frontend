...
export default function ExpenseForm({ user, onSuccess }: ExpenseFormProps) {
  ...
  const handleFileUpload = async (files: FileList | File[]) => {
    ...
    setProcessingProgress(`Processing & uploading ${fileArray.length} file(s)...`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!amount || parsedAmount <= 0 || !vendor) {
      setMessage({ type: "error", text: "Please enter a valid amount greater than ₹0 and fill in all required fields." });
      return;
    }
            ...
            >
              {isProcessingFiles ? (
                <div className="flex flex-col items-center py-2">
                  ...
                  <p className="text-xs font-bold text-slate-700">{processingProgress || "Processing images..."}</p>
                  <p className="text-[10px] text-slate-400 mt-1">Auto-compressing multiple bill receipts for fast submission...</p>
                </div>
              ) : (
                <>
            ...
            {uploadedBills.length > 0 && (
              <div className="mt-4 space-y-2">
                <div className="flex items-center justify-between text-xs font-bold text-slate-700">
                  <span>Attached Bill Receipts ({uploadedBills.length})</span>
            ...
            >
              <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
                ...
                <div className="flex items-center gap-2">
                  {uploadedBills.length > 1 && (
                    <div className="flex items-center gap-1 bg-slate-200/70 p-0.5 rounded-lg mr-2">
                      <button
                        type="button"
                        onClick={() => {
                          const idx = uploadedBills.findIndex(b => b.id === previewBill.id);
                          if (idx > 0) {
                            setPreviewBill(uploadedBills[idx - 1]);
                          } else {
                            setPreviewBill(uploadedBills[uploadedBills.length - 1]);
                          }
                        }}
                      ...
                      <span className="text-[11px] font-mono font-bold text-slate-700 px-1.5">
                        {uploadedBills.findIndex(b => b.id === previewBill.id) + 1}/{uploadedBills.length}
                      </span>
                      <button
                        type="button"
                        onClick={() => {
                          const idx = uploadedBills.findIndex(b => b.id === previewBill.id);
                          if (idx < uploadedBills.length - 1) {
                            setPreviewBill(uploadedBills[idx + 1]);
                          } else {
                            setPreviewBill(uploadedBills[0]);
                          }
                        }}
                        className="p-1 text-slate-600 hover:text-slate-900 hover:bg-white rounded transition cursor-pointer"
